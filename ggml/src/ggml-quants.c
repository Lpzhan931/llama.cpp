...
void quantize_row_q4_0_64_ref(const float * GGML_RESTRICT x, block_q4_0_64 * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        // 循环次数从 16 变为 32 (qk/2)
        for (int j = 0; j < qk/2; ++j) {
            // 原始 Q4_0 是前16个和后16个打包
            // 这里变成 前32个 和 后32个 打包
            const float x0 = x[i*qk + 0    + j]*id;
            const float x1 = x[i*qk + qk/2 + j]*id; // 偏移量变为 32
...
// NOTE
static void quantize_row_q4_0_64_impl(const float * GGML_RESTRICT x, block_q4_0_64 * GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
    // 强制检查块大小
    static_assert(QK4_0_64 == 64, "QK4_0_64 must be 64"); 

    if (!quant_weights) {
        quantize_row_q4_0_64_ref(x, y, n_per_row);
        return;
    }

    // 如果有权重 (quant_weights)，这部分逻辑用于提高量化精度 (Make Quants)
    // 简单起见，如果只为了测试效率，可以直接调用 ref 实现。
    // 如果需要高质量量化，需要适配 make_qx_quants 的参数。
    // 这里给出一个基于 q4_0_impl 修改的版本：

    float weight[QK4_0_64];
    int8_t L[QK4_0_64];
    ...
    const int64_t nb = n_per_row/QK4_0_64;
    for (int ib = 0; ib < nb; ++ib) {
        const float * xb = x + QK4_0_64 * ib;
        const float * qw = quant_weights + QK4_0_64 * ib;
        for (int j = 0; j < QK4_0_64; ++j) weight[j] = qw[j] * sqrtf(sigma2 + xb[j]*xb[j]);
        
        // 注意：make_qx_quants 的 nmax 仍为 8 (因为是4bit, -8到7)
        float d = make_qx_quants(QK4_0_64, 8, xb, L, 1, weight);
        
        y[ib].d = GGML_FP32_TO_FP16(d);
        for (int j = 0; j < QK4_0_64/2; ++j) {
            y[ib].qs[j] = L[j] | (L[j + QK4_0_64/2] << 4);
        }
    }
}
...
// NOTE
size_t quantize_q4_0_64(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    // 注意这里需要 GGML_TYPE_Q4_0_64 枚举值，需在 ggml.h 中定义
    if (!quant_weights) {
        quantize_row_q4_0_64_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q4_0_64, n_per_row);
    }
    size_t row_size = ggml_row_size(GGML_TYPE_Q4_0_64, n_per_row);
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q4_0_64_impl(src, (block_q4_0_64*)qrow, n_per_row, quant_weights);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
